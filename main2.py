...
with st.spinner('Loading...'):
    ...
    hourly_df.rename(
        columns={'temperature_2m': 'Temperature °C'}, inplace=True)
    hourly_df.rename(
        columns={'precipitation': 'Precipitation mm'}, inplace=True)
    ...
    week_ahead = pd.to_datetime(
        hourly_df['Week ahead'], format="%Y-%m-%dT%H:%M")
